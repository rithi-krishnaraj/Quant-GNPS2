...
def extract_skyline_transitions_from_mgf(mgf_files, compound_df, polarity_mode, fragment_dedup_ppm=5, compound_match_ppm_tolerance=10):
    """
    Generates Skyline-compatible transition lists by extracting fragment ions from MGF files.
    The workflow involves parsing MGF spectra, matching precursors to target compounds
    within a specified m/z tolerance, and selecting the most abundant fragments.
    It also performs deduplication to ensure unique and high-quality transitions.
    Returns a DataFrame containing the formatted Skyline transition data.
    """
...
def build_mzml_figure(mzml_file, target_df, title_base, xic_ppm_tolerance):
    """
    Extracts Total Ion Chromatograms (TIC) and Extracted Ion Chromatograms (XICs)
    from an mzML file for visualization. This function helps in qualitatively
    assessing the presence and chromatographic profile of targeted compounds
    within the raw LC-MS data. It generates a multi-panel Matplotlib figure
    showing overall TIC and individual XICs for the top compounds.
    Returns the Matplotlib figure object, or None if parsing fails or no MS1 scans are found.
    """
...
if st.session_state.input_mode == "Manual Input":
    ...
    else: # Option 2: GNPS2 Task ID Test Data
        f_compounds = None
...
if current_mode == "GNPS2 Task ID":
    if fetched and fetched.get('task_id') == st.session_state.get('gnps_task_id'):
        gnps_status = "✓ Data Fetched"
elif current_mode == "Test Data":
    # Option 2 specifically uses this task ID
    if fetched and fetched.get('task_id') == "20a59d94003c41168a5186a00e2ac086":
        gnps_status = "✓ Test Data Fetched"
...
status_data = {
    ...
    "Status": [
        ...
        f"✓ {len(f_gnps_pos)} files" if (isinstance(f_gnps_pos, list) and len(f_gnps_pos) > 0) else (gnps_status if current_mode in ["GNPS2 Task ID", "Test Data"] else "✗ Not uploaded"),
        f"✓ {len(f_mzmine_pos)} files" if (isinstance(f_mzmine_pos, list) and len(f_mzmine_pos) > 0) else (gnps_status if current_mode in ["GNPS2 Task ID", "Test Data"] else "✗ Not uploaded"),
        "✓ Uploaded" if f_mzml_pos else "✗ Not uploaded",
        f"✓ {len(f_gnps_neg)} files" if (isinstance(f_gnps_neg, list) and len(f_gnps_neg) > 0) else (gnps_status if current_mode in ["GNPS2 Task ID", "Test Data"] else "✗ Not uploaded"),
        f"✓ {len(f_mzmine_neg)} files" if (isinstance(f_mzmine_neg, list) and len(f_mzmine_neg) > 0) else (gnps_status if current_mode in ["GNPS2 Task ID", "Test Data"] else "✗ Not uploaded"),
    ...
    with st.status("Evaluating Method Constraints...", expanded=True) as status:
        try:
            ...
            if effective_mode == "Test Data":
                effective_mode = "Manual Input" if st.session_state.test_data_mode == "Option 1: Manual Input Test Data" else "GNPS2 Task ID"
