...
def create_thermo_fisher_csv(df, rt_window_mode='composite_margin'):
    ...
    - composite_margin/symmetric_margin/hybrid/composite_margin_pct: t start (min), t stop (min)
...
def parse_mgf_spectrum_list(mgf_file_content):
    ...
    Only extracts MS/MS spectra (MSLEVEL=2).
    ...
    for line in mgf_file_content.decode('utf-8', errors='ignore').split('\n'):
        ...
        if line == "END IONS":
            # Only keep MS/MS spectra (MSLEVEL=2)
            if current_spectrum and current_spectrum['pepmass'] is not None and current_spectrum['mslevel'] == 2:
            ...
            for spectrum in spectra:
                ...
                if precursor_mz is not None and len(fragments) > 0:
                    ...
                    if matched_compound:
                        ...
                        # 2. If charge=1, exclude fragments with m/z >= precursor m/z (fragments should be lighter)
                        # 3. If no charge given, skip this spectrum
                        spectrum_charge = spectrum.get('charge', None)
                        if spectrum_charge is None:
                            continue  # Skip if no charge information
                        ...
                        for (_, mz, intensity) in fragments_sorted:
                            ...
                            # If charge is +1, fragment m/z must be less than precursor m/z
                            if spectrum_charge == 1 and mz >= precursor_mz:
                                continue
...
def calculate_rt_window(peak_rt, composite_rt_min, composite_rt_max, rt_window_mode,
                         rt_margin_min=1.0, rt_margin_pct=None, expected_peak_width_min=0.167):
    """
    ...
    - 'hybrid': Option 3 — Use 1a if peak_width <= expected_peak_width_min, else use 2b
    ...
    if rt_window_mode == 'hybrid':
        # Option 3: Hybrid - use 1a for narrow peaks (<=expected), 2b for wide peaks (>expected)
...
def process_polarity(gnps_df, mzmine_df, polarity, targets_df, hcd_energies,
    ...
    for compound_name, group in merged.groupby('Standardized_Compound'):
        ...
        t_start, t_stop, exp_peak_width_sec = calculate_rt_window(
            ...
            rt_margin_min, rt_margin_pct, expected_peak_width_sec
        )
...
def build_rt_alignment_figure(df, title, rt_window_mode="Composite range ± margin"):
    ...
    for grp_num, group_df in df.groupby('Multiplex_Group'):
        ...
        ax.set_xlabel("Retention time (min)")
        mode_label = "📊 Composite ± Margin" if rt_window_mode == "Composite range ± margin" else "⭕ Symmetric"
...
def build_points_per_peak_figure(df, title):
    ...
    for grp_num, group_df in df.groupby('Multiplex_Group'):
        ...
        def get_color(pts):
            if pts > 10: return '#2ca02c'      
            elif pts >= 8: return '#ff7f0e'    
            elif pts >= 2: return '#d62728'    
            else: return '#8b0000'             
...
with st.sidebar:
    ...
    if rt_mode_type == "Start/End Time":
        ...
        elif rt_calculation == "Option 3: Hybrid":
...
if generate_skyline:
    if polarity_mode in ["Positive & Negative", "Positive Only"]:
        mgf_pos_files = st.file_uploader("GNPS MGF — ESI+ (.mgf)", type=["mgf"], accept_multiple_files=True, key="mgf_pos")
    if polarity_mode in ["Positive & Negative", "Negative Only"]:
        mgf_neg_files = st.file_uploader("GNPS MGF — ESI− (.mgf)", type=["mgf"], accept_multiple_files=True, key="mgf_neg")
